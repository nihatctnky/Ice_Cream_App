...
const Card = ({ item }) => {
    ...
    const [selectedType, setSelectedType] = useState(item.type || null); // Varsayılan olarak item.type al

    const handleType = (type) => {
        setSelectedType(type === selectedType ? null : type); // Seçim yapıldığında güncelle
    };

    const handleBasket = () => {
        if (!selectedType) {
            console.error("Lütfen bir tür seçin.");
            return; // Eğer type seçilmemişse sepet eklemeden çık
        }
